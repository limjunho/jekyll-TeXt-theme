...
이 문제에서 사용할 해킹 기법은 **buffer overflow**이며 buffer over flow(bof)란  
직역하면 버퍼가 넘쳐흐르다로 프로그래머가 지정한 곳 바깥(의도하지 않은 메모리공간)에 해커가 데이터를 저장시키거나 유출시키는 해킹기법이다.  
